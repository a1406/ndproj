...
static NEUINT32 SMALL_PRIMES[] = { 3, 5, 7, 11 };
#define SMALL_PRIME_COUNT 4

static NEINT32 ProbablePrime PROTO_LIST ((NN_DIGIT *, NEUINT32));
static NEINT32 SmallFactor PROTO_LIST ((NN_DIGIT *, NEUINT32));
static NEINT32 FermatTest PROTO_LIST ((NN_DIGIT *, NEUINT32));
...
/* Generates a probable prime a between b and c such that a-1 is
   ...
   unsuccessful.
 */
NEINT32 GeneratePrime (a, b, c, d, digits, randomStruct)
NN_DIGIT *a, *b, *c, *d;
NEUINT32 digits;
R_RANDOM_STRUCT *randomStruct;
{
  NEINT32 status;
  NEUINT8 block[MAX_NN_DIGITS * NN_DIGIT_LEN];
 ...
 */
static NEINT32 ProbablePrime (a, aDigits)
NN_DIGIT *a;
NEUINT32 aDigits;
{
...
/* Returns nonzero iff a has a prime factor in SMALL_PRIMES.
   ...
   Assumes aDigits < MAX_NN_DIGITS.
 */
static NEINT32 SmallFactor (a, aDigits)
NN_DIGIT *a;
NEUINT32 aDigits;
{
  NEINT32 status;
...
/* Returns nonzero iff a passes Fermat's test for witness 2.
   ...
   Assumes aDigits < MAX_NN_DIGITS.
 */
static NEINT32 FermatTest (a, aDigits)
NN_DIGIT *a;
NEUINT32 aDigits;
{
  NEINT32 status;
