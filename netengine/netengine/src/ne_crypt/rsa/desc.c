...
static NEUINT8 TOTAL_ROTATIONS[16] = {
  1, 2, 4, 6, 8, 10, 12, 14, 15, 17, 19, 21, 23, 25, 27, 28
};

static NEUINT8 PC2[48] = {
...
static void Unpack PROTO_LIST ((NEUINT8 *, UINT4 *));
static void Pack PROTO_LIST ((UINT4 *, NEUINT8 *));
static void DESKey PROTO_LIST ((UINT4 *, NEUINT8 *, NEINT32));
static void CookKey PROTO_LIST ((UINT4 *, UINT4 *, NEINT32));
...
NEUINT8 key[8];                                                /* key */
NEUINT8 iv[8];                                 /* initializing vector */
...
/* DES-CBC block update operation. Continues a DES-CBC encryption
   ...
   the context.
 */
NEINT32 DES_CBCUpdate (context, output, input, len)
DES_CBC_CTX *context;                                            /* context */
...
NEUINT8 key[24];                              /* DES key and whiteners */
NEUINT8 iv[8];                              /* DES initializing vector */
NEINT32 encrypt;                      /* encrypt flag (1 = encrypt, 0 = decrypt) */
{  
  /* Copy encrypt flag to context.
   */
...
/* DESX-CBC block update operation. Continues a DESX-CBC encryption
   ...
   the context.
 */
NEINT32 DESX_CBCUpdate (context, output, input, len)
...
NEINT32 DES3_CBCUpdate (context, output, input, len)
DES3_CBC_CTX *context;                                           /* context */
NEUINT8 *output;                                      /* output block */
NEUINT8 *input;                                        /* input block */
NEUINT32 len;                      /* length of input and output blocks */
...
UINT4 *outof;
{
  *into++ = (NEUINT8)((*outof >> 24) & 0xffL);
  *into++ = (NEUINT8)((*outof >> 16) & 0xffL);
  *into++ = (NEUINT8)((*outof >>  8) & 0xffL);
  *into++ = (NEUINT8)( *outof++      & 0xffL);
  *into++ = (NEUINT8)((*outof >> 24) & 0xffL);
  *into++ = (NEUINT8)((*outof >> 16) & 0xffL);
  *into++ = (NEUINT8)((*outof >>  8) & 0xffL);
  *into = (NEUINT8)( *outof        & 0xffL);
...
NEINT32 encrypt;
{
  UINT4 kn[32];
  NEINT32 i, j, l, m, n;
  NEUINT8 pc1m[56], pcr[56];

  for (j = 0; j < 56; j++) {
    l = PC1[j];
    m = l & 07;
    pc1m[j] = (NEUINT8)((key[l >> 3] & BYTE_BIT[m]) ? 1 : 0);
  }
  for (i = 0; i < 16; i++) {
    m = i << 1;
    n = m + 1;
